...
class TestUnitRoleAssignment:
    def test_assigned_with_roles(self):
        a = UnitRoleAssignment(status="assigned", roles=("control", "storage"))
        assert a.status == "assigned"
        assert a.roles == ("control", "storage")
        assert a.message is None

    def test_pending_defaults(self):
        a = UnitRoleAssignment(status="pending")
        assert a.status == "pending"
        assert a.roles == ()
        assert a.message is None

    def test_error_with_message(self):
        a = UnitRoleAssignment(status="error", message="unit not found in topology")
    ...
    def test_frozen(self):
        ...
        with pytest.raises(AttributeError):
            a.status = "pending"  # type: ignore[misc]
    ...
    def test_to_dict_error(self):
        a = UnitRoleAssignment(status="error", message="bad")
        d = a.to_dict()
    ...
    def test_equality(self):
        a = UnitRoleAssignment(status="assigned", roles=("control",))
        b = UnitRoleAssignment(status="assigned", roles=("control",))
        assert a == b

    def test_inequality(self):
        a = UnitRoleAssignment(status="assigned", roles=("control",))
        b = UnitRoleAssignment(status="assigned", roles=("storage",))
        assert a != b
